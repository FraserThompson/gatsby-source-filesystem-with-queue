        ...
        switch (op) {
          ...
          case `upsert`:
            return new Promise((resolve, reject) => {
              fileProcessingQueue.push(path, (err => {
                if (err) {
                  console.error(err);
                  reject();
                } else {
...
See docs here - https://www.gatsbyjs.com/plugins/gatsby-source-filesystem/
  ...
  const createAndProcessNode = (path, callback) => {
    createFileNode(path, api.createNodeId, pluginOptions).then(fileNode => {
      api.actions.createNode(fileNode)
      callback();
    });
  };
  ...
  const fsMachine = createFSMachine(api, fileProcessingQueue)
